...
class TaskTableViewController: UITableViewController {
    ...
    @IBAction func popSample(sender:AnyObject) {
        ...
        imageView.frame = CGRect(x: 5, y: 20, width: 260, height: 100)
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        title = course.name
        earnedMark.text = "\(course.getCurrentPoint())"
        past.text = "\(course.pastRatio())"
        if course.pastRatio() > 0 {
            effort.text = "\((course.getCurrentPoint()/course.pastRatio())*100)%"
        } else {
            effort.text = "0.0%"
        }
        
        //tableView.backgroundColor = UIColor(white: 1.0, alpha: 1)
        //tableView.tableFooterView = UIView(frame: CGRectZero)
        //tableView.separatorColor = UIColor(white: 0, alpha: 1)
